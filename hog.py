...
def free_bacon(score):
    ...
    while x > 0:
        digit, x = x % 10, x // 10
        self_score += (-1)**n*digit
        n += 1
    self_score = 1 + abs(self_score)
    return self_score
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            if feral_hogs:
                if abs(num_roll-points0)==2:
                    score0 += 3
            points0 = take_turn(num_roll, score1, dice)
            score0 += points0
        else:
            num_roll = strategy1(score1, score0)
            if feral_hogs:
                if abs(num_roll-points1)==2:
                    score1 += 3
            points1 = take_turn(num_roll, score0, dice)
            score1 += points1
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    max_roll=1
    for i in range(1,11):
        point=averaged_roll_dice(i,dice)
        if point>max_score:
            max_score=point
            max_roll=i
    return max_roll
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    if (is_swap(new_score,opponent_score) and new_score<opponent_score) or (not is_swap(new_score,opponent_score) and free_bacon(opponent_score)>=margin):
        return 0
    else:
        return num_rolls  # Replace this statement
    # END PROBLEM 11
